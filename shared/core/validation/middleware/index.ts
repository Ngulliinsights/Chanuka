      ...
      if (config.body && req.body !== undefined) {
        try {
          ...
          if (error instanceof ValidationError) {
            ...
            (newError as unknown).errors = prefixedErrors;
            throw newError;
          }
          throw error;
        }
...
export class ValidationMiddleware {
   ...
   */
  static body(schema: ZodSchema, options?: unknown) {
   ...
   */
  static query(schema: ZodSchema, options?: unknown) {
   ...
   */
  static params(schema: ZodSchema, options?: unknown) {
   ...
   */
  static headers(schema: ZodSchema, options?: unknown) {
 ...
 */
export function validateClass(_defaultOptions?: unknown) {
 ...
 */
export function validateBatch(schema: ZodSchema, options?: unknown) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
  ...
  options?: any
    ...
    try {
      ...
      if (fileSchema && (req as unknown).file) {
        const fileData = {
          filename: (req as unknown).file.originalname,
          mimetype: (req as unknown).file.mimetype,
          size: (req as unknown).file.size,
        };
      ...
      if (fileSchema && (req as unknown).files && Array.isArray((req as unknown).files)) {
        const filesData = (req as unknown).files.map((file: unknown) => ({
 ...
 */
export function getValidatedData(req: Request): unknown {
  return (req as unknown).validated || {};
}
