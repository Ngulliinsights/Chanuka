...
// Local lightweight shims for UI components so this shared package doesn't depend on client-only UI modules.
const Button: React.FC<any> = ({ children, className = '', ...rest }) => (
  <button {...rest} className={`inline-flex items-center justify-center px-3 py-2 rounded ${className}`}>{children}</button>
);

const Card: React.FC<any> = ({ children, className = '', ...rest }) => (
  <div {...rest} className={`bg-white rounded shadow ${className}`}>{children}</div>
);

const CardHeader: React.FC<any> = ({ children, className = '', ...rest }) => (
  <div {...rest} className={`px-4 py-3 border-b ${className}`}>{children}</div>
);
...
const CardTitle: React.FC<any> = ({ children, className = '', ...rest }) => (
  <h3 {...rest} className={`text-lg font-semibold ${className}`}>{children}</h3>
);

const CardDescription: React.FC<any> = ({ children, className = '', ...rest }) => (
  <p {...rest} className={`text-sm text-gray-600 ${className}`}>{children}</p>
);

const Logo: React.FC<any> = ({ size = 'md', showText = true }) => (
  <div className="flex items-center space-x-2">
    <img src="/Chanuka_logo.svg" alt="Chanuka" className={size === 'md' ? 'h-8 w-8' : 'h-6 w-6'} />
    {showText && <span className="font-bold">Chanuka</span>}
  </div>
);
