...
import { z } from 'zod';
import { GraphAnalyzer, Statistics, Cache } from './shared-utils';
...
export class InfluenceMapper {
    ...
    for (const [nodeId, node] of network.nodes) {
      ...
      for (let i = 0; i < neighbors.length; i++) {
        for (let j = i + 1; j < neighbors.length; j++) {
          const adj1 = network.adjacencyMap.get(neighbors[i]);
          if (adj1 && adj1.has(neighbors[j])) {
            triangles++;
          }
        }
      }
  ...
  private determineFlowType(edge: EdgeData): 'financial' | 'informational' | 'political' | 'social' {
    ...
    return mapping[edge.type as keyof typeof mapping] || 'informational';
  }
  ...
  private analyzeLobbyingActivity(nodeId: string, network: NetworkGraph, input: InfluenceInput) {
    ...
    for (const targetId of node.outgoingConnections) {
  ...
  private identifyCoordinatedLobbying(network: NetworkGraph, input: InfluenceInput, activities: any[]) {
    ...
    for (const [targetId, lobbyists] of lobbyistGroups) {
      if (lobbyists.size >= 3) {
