...
import {
  ...
  disclosureTypeEnum, moderationContentTypeEnum, flagTypeEnum, moderationStatusEnum, moderationActionTypeEnum,
  securityResultEnum, complianceCheckTypeEnum, threatTypeEnum, threatSourceEnum, regulationStatusEnum,
  syncStatusEnum, syncErrorLevelEnum, conflictResolutionEnum, notificationTypeEnum, attackPatternTypeEnum,
  securityAlertStatusEnum, verificationTypeEnum
} from "./enum";
...
export const bill = pgTable("bill", {
  ...
  // Drizzle doesn't provide a tsvector column builder in this version, so use text here to keep TypeScript happy.
  // The actual DB migrations create the tsvector column and GIN index.
  ...
  viewCountIdx: index("bill_view_count_idx").on(table.viewCount), // <-- ADDED FROM schema1.ts
  searchVectorIdx: index("bill_search_vector_idx").using("gin", table.searchVector), // <-- ADDED FROM schema1.ts
