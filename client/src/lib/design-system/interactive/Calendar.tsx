...
interface EnhancedCalendarProps
  extends Omit<CalendarProps, 'className' | 'selected' | 'onSelect' | 'disabled'>,
    DateValidationProps {
...
const EnhancedCalendar = React.forwardRef<HTMLDivElement, EnhancedCalendarProps>(
  (
    {
      className,
      minDate,
    ...
    const validateSelectedDate = React.useCallback(
      (date: Date | Date[] | DateRange | undefined): ValidationState => {
        if (!showValidation) {
          ...
          if (date) {
            ...
            if ('from' in date) {
              ...
              if (range.from) {
                ...
                const result = safeValidateDate(range.from);
                if (!result) {
                  return {
                    isValid: false,
              ...
              if (range.to) {
                ...
                if (
                  disabledDates.some(
                    (disabledDate: Date) => disabledDate.toDateString() === range.to!.toDateString()
                  )
                ) {
                  return {
                    isValid: false,
                ...
                const result = safeValidateDate(range.to);
                if (!result) {
                  return {
                    isValid: false,
                    error: 'Invalid end date selection',
                    touched: validationState.touched,
                  };
                }
              }
              ...
              if (
                disabledDates.some(
                  (disabledDate: Date) => disabledDate.toDateString() === singleDate.toDateString()
                )
              ) {
                return {
                  isValid: false,
              ...
              const result = safeValidateDate(singleDate);
              if (!result) {
                return {
                  isValid: false,
      ...
      [showValidation, required, disabledDates, validationState.touched]
    );

    const handleValidationError = React.useCallback(
      async (error: UIDateError) => {
        try {
    ...
    const handleSelect = React.useCallback(
      (date: Date | Date[] | undefined) => {
        const validationResult = validateSelectedDate(date);
    ...
    const isDateDisabled = React.useCallback(
      (date: Date) => {
        ...
        if (
          disabledDates.some(
            (disabledDate: Date) => disabledDate.toDateString() === date.toDateString()
          )
        ) {
          return true;
        }
    ...
    return (
      <div ref={ref} className="space-y-2">
        <DayPicker
          ...
          className={cn('p-3', hasError && 'border border-destructive rounded-md', className)}
          classNames={{
            months: 'flex flex-col sm:flex-row space-y-4 sm:space-x-4 sm:space-y-0',
            month: 'space-y-4',
            caption: 'flex justify-center pt-1 relative items-center',
          ...
          mode={selected && typeof selected === 'object' && 'from' in selected ? 'range' : 'single'}
