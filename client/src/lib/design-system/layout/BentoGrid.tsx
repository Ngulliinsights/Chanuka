...
export const BentoGrid = React.memo<BentoGridProps>(
  ({ children, className, cols = 3, gap = 'md', as: Component = 'div' }) => {
    const gapClasses = {
      sm: 'gap-2',
      md: 'gap-4',
      lg: 'gap-6',
    ...
    return (
      <Component className={cn('grid w-full', gapClasses[gap], colClasses[cols], className)}>
        {children}
      </Component>
    );
  }
...
export const BentoItem = React.memo<BentoItemProps>(
  ({ children, className, span = 1, rowSpan = 1, onClick, 'aria-label': ariaLabel }) => {
