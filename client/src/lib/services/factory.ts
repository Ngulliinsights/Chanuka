...
export class ServiceContainer {
      ...
      for (const [id, registration] of this.registrations) {
        if (registration.lifecycle === ServiceLifecycle.SINGLETON) {
          try {
            ...
            throw ServiceErrorFactory.createDependencyError(id, `Failed to initialize singleton service: ${errorMessage}`, 'initialize', { originalError: error });
          }
        }
      }
  ...
  private createInstance(registration: ServiceRegistration): unknown {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw ServiceErrorFactory.createDependencyError(registration.id, `Failed to create service instance: ${errorMessage}`, 'createInstance', { originalError: error });
    }
  }
...
export class ServiceFactory {
  ...
  static configure(config: {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      throw ServiceErrorFactory.createConfigurationError('configure', `Failed to configure service factory: ${errorMessage}`, 'configure', { originalError: error });
    }
  }
