...
export const CardSkeleton = React.memo<CardSkeletonProps>(
  ({
    ...
    return (
      <div
        className={cn('p-6 border border-gray-200 rounded-lg bg-white', className)}
        role="presentation"
        aria-hidden="true"
      >
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1 min-w-0">
            {showTitle && <Skeleton className="h-5 w-3/4 mb-2" />}
            {showSubtitle && <Skeleton className="h-4 w-1/2 mb-3" />}
            {showDescription && (
