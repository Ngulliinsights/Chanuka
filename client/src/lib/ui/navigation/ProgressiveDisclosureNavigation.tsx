...
import {
  ...
  DropdownMenuItem,
  DropdownMenuTrigger,
} from '@client/lib/design-system/interactive/DropdownMenu';
...
const ComplexityIndicator: React.FC<{ complexity: 1 | 2 | 3; className?: string }> = ({
  ...
  <div
    ...
    aria-label={`Complexity level ${complexity} out of 3`}
  >
    {Array.from({ length: 3 }, (_, i) => (
      <div
        ...
        className={cn(
          ...
          i < complexity ? 'bg-primary' : 'bg-gray-300'
        )}
      />
    ))}
  </div>
...
export const ProgressiveDisclosureNavigation = React.memo<ProgressiveDisclosureNavigationProps>(
  ({ sections, readingPaths = [], currentSectionId, onSectionChange, className }) => {
    ...
    return (
      <div ref={containerRef} className={cn('space-y-4', className)}>
        ...
        {/* Navigation sections */}
        <div className="space-y-2">{sections.map(section => renderSection(section))}</div>
      </div>
    );
  }
);
