...
// Provide a thin `useDiscussionStore` hook for legacy components that expect
// a single hook returning threads, comments and a small set of imperative
// functions. These are thin wrappers around Redux selectors and thunks.
// ---------------------------------------------------------------------------

import { useDispatch, useSelector } from 'react-redux';

export function useDiscussionStore() {
  const dispatch = useDispatch();
  const discussion = useSelector(selectDiscussionState);

  const threads = discussion.threads;
  const comments = discussion.comments;

  // Placeholder typing indicators array (slice doesn't track typing indicators)
  const typingIndicators: any[] = [];

  const initializeRealTime = async () => {
    // In this minimal compatibility layer we'll just mark connected true.
    try {
      dispatch(setConnected(true) as any);
    } catch (e) {
      // ignore
    }
  ...
  const cleanupRealTime = () => {
    try {
      dispatch(setConnected(false) as any);
    } catch (e) {
      // ignore
    }
  };

  const loadThread = async (billId: number) => {
    // Reuse existing thunk to load discussion data
    // Cast dispatch to any to avoid stricter AppDispatch typing here
    return (dispatch as any)(loadDiscussionData(billId));
  };

  const loadComments = async (billId: number) => {
    // No dedicated comments-only thunk — reuse loadDiscussionData for compatibility
    return (dispatch as any)(loadDiscussionData(billId));
  };
...
// Backwards-compatibility selectors hook
// Provides a similar surface to older code that used a single hook
// to access discussion state and simple action helpers.
import { useDispatch, useSelector } from 'react-redux';

export function useDiscussionSelectors() {
  const dispatch = useDispatch();
  const discussion = useSelector(selectDiscussionState);

  const loading = discussion.loading;
  const error = discussion.error;

  const getThread = (billId: number) => discussion.threads[billId] || null;
  const getThreadComments = (billId: number) => (discussion.threads[billId]?.comments) || [];
  const getTypingIndicators = (billId: number) => [] as any[]; // slice doesn't track typing indicators

  const setLoadingLocal = (v: boolean) => dispatch(setLoading(v));
  const setErrorLocal = (msg: string | null) => dispatch(setError(msg));
  const setThreadLocal = (billId: number, thread: any) => dispatch(discussionSlice.actions.setThread({ billId, thread } as any));

  const handleCommentUpdate = (event: any) => {
    if (!event) return;
    if (event.type === 'comment_added' && event.comment) {
      dispatch(discussionSlice.actions.addComment(event.comment));
    } else if (event.type === 'comment_updated' && event.commentId) {
      dispatch(discussionSlice.actions.updateComment({ commentId: event.commentId, updates: event.comment || {} }));
    } else if (event.type === 'comment_removed' && event.commentId) {
      dispatch(discussionSlice.actions.removeComment(event.commentId));
    }
  };

  const handleModerationEvent = (event: any) => {
    // Minimal handling: if a comment was moderated, update its status
    if (!event) return;
    if (event.type === 'comment_moderated' && event.commentId && event.action) {
      const updates: any = {};
      if (event.action === 'hide') updates.status = 'hidden';
      if (event.action === 'remove') updates.status = 'removed';
      dispatch(discussionSlice.actions.updateComment({ commentId: event.commentId, updates }));
    }
  };

  const updateTypingIndicators = (_: any) => {
    // No-op in slice; typing handled elsewhere
  };

  // Store action wrappers
  const addCommentLocal = (comment: any) => dispatch(discussionSlice.actions.addComment(comment));
  const updateCommentLocal = (commentId: string, updates: any) => dispatch(discussionSlice.actions.updateComment({ commentId, updates }));
  const removeCommentLocal = (commentId: string) => dispatch(discussionSlice.actions.removeComment(commentId));
  const voteCommentLocal = (commentId: string, voteType: 'up' | 'down', userId: string) => dispatch(discussionSlice.actions.voteComment({ commentId, voteType, userId }));
  const addReportLocal = (report: any) => { /* reports handled via thunks */ };
  const addModerationActionLocal = (action: any) => { /* minimal no-op */ };
  ...
  return {
    ...
    removeComment: removeCommentLocal,
    voteComment: voteCommentLocal,
    addReport: addReportLocal,
    addModerationAction: addModerationActionLocal,
  };
}