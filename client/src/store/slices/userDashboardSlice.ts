...
export const requestDataExport = createAsyncThunk(
  ...
  async (request: DataExportRequest, { rejectWithValue }) => {
    try {
      ...
      switch (type) {
        case 'bill_status_change':
          if (state.dashboardData) {
            ...
            if (billIndex !== -1) {
              state.dashboardData.trackedBills[billIndex].status = data.newStatus;
        ...
        case 'new_recommendation':
          if (state.dashboardData) {
            state.dashboardData.recommendations.unshift(data.recommendation);
