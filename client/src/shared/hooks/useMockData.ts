 ...
 */
export function useMockData<T>(key: keyof typeof dataLoaders, options: UseMockDataOptions = {}) {
  ...
  const handleRealTimeEvent = useCallback(
    (event: RealTimeEvent) => {
      ...
      if (
        keyRelevantEvents &&
        (keyRelevantEvents.includes('*') || keyRelevantEvents.includes(event.type))
      ) {
 ...
 */
export function useRealTimeEvents(eventTypes?: string[], maxEvents: number = 50) {
