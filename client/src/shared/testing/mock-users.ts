...
class MockUserDataService {
  ...
  generateMockUser(overrides: Partial<MockUser> = {}): MockUser {
    ...
    const constituencies = this.CONSTITUENCIES[county as keyof typeof this.CONSTITUENCIES] || [
      'Central',
    ];
...
export const mockUserUtils = {
  ...
  getUserEngagementLevel(user: MockUser): 'low' | 'medium' | 'high' {
    const score =
      user.stats.commentsPosted + user.stats.votesParticipated * 2 + user.stats.billsFollowed;
