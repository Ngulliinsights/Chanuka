...
export default function CheckpointDashboard({ projectId }: CheckpointDashboardProps) {
  ...
  const updateCheckpointMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<Checkpoint> }) => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`${colors[status as keyof typeof colors]} px-2 py-1 rounded text-xs font-medium`}>
    ...
    <section className="bg-background rounded-lg border border-border p-6">
      ...
      <div className="space-y-4">
        {checkpoints?.map((checkpoint) => {
          ...
          return (
            <div key={checkpoint.id} className={`border border-border rounded-lg p-4 bg-${statusColor}-50 border-${statusColor}-200`}>
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-3">
                  <div className="mt-1">
                    <div className={`w-3 h-3 bg-${statusColor}-500 rounded-full ${isInProgress ? "animate-pulse" : ""}`}></div>
                  </div>
                  <div>
                    <h3 className={`font-medium text-${statusColor}-900`}>{checkpoint.name}</h3>
                    <p className={`text-sm text-${statusColor}-700 mt-1`}>{checkpoint.description}</p>
                    <div className={`flex items-center space-x-4 mt-2 text-xs text-${statusColor}-600`}>
                      <span>
                        {checkpoint.status === "completed" ? "Completed" : "Started"}: {
                      ...
                      {checkpoint.targetDate && (
                        <span>
                          Target: {new Date(checkpoint.targetDate).toLocaleDateString('en-US', { 
                            month: 'short', 
                            day: 'numeric', 
                            year: 'numeric' 
                          })}
                    ...
                    {isInProgress && checkpoint.metrics && (
                      <div className="mt-3">
                        <div className="flex items-center justify-between text-xs text-blue-600 mb-1">
                          <span>Progress</span>
                          <span>{checkpoint.metrics.features_completed}/{checkpoint.metrics.features_total} features</span>
                        </div>
                        <div className="w-full bg-blue-200 rounded-full h-2">
                          <div 
                            className="bg-blue-600 h-2 rounded-full" 
                            style={{ width: `${(checkpoint.metrics.features_completed / checkpoint.metrics.features_total) * 100}%` }}
                          ></div>
                        </div>
                      </div>
                    )}
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button className={`p-1 hover:bg-${statusColor}-200 rounded`}>
                    {isInProgress ? (
                      ...
                      <ExternalLink className={`w-4 h-4 text-${statusColor}-600`} />
                    )}
                  </button>
                </div>
              </div>
                ...
                <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="flex-1">
                      <h4 className="text-sm font-medium text-amber-900">Checkpoint Review Available</h4>
                      <p className="text-xs text-amber-700 mt-1">Current phase eligible for pivot evaluation. Review metrics and decide on next direction.</p>
