...
describe('EnhancedForm', () => {
  ...
  describe('Accessibility', () => {
    ...
    it('makes error fields focusable', async () => {
      render(
        <EnhancedForm 
          schema={TestSchema}
          config={{ showErrorSummary: true, scrollToFirstError: true }}
        >
          <input name="name" />
          <input name="email" />
          <button type="submit">Submit</button>
      ...
      await waitFor(() => {
        ...
        expect(errorLinks.length).toBeGreaterThan(1); // Submit button + error links
      });
    });
  });
