...
export function ConstitutionalContext({ 
  ...
  const [selectedConstitutionalArticle, setSelectedConstitutionalArticle] = useState<string | null>(null);
  ...
  const getImpactColor = (impact: string) => {
    switch (impact) {
      ...
      case 'conflicts': return 'text-red-600 bg-red-50 border-red-200';
  ...
  const getImpactIcon = (impact: string) => {
    switch (impact) {
      case 'supports': return <CheckCircle className="h-4 w-4 text-green-600" />;
      case 'conflicts': return <AlertTriangle className="h-4 w-4 text-red-600" />;
      case 'neutral': return <Info className="h-4 w-4 text-blue-600" />;
      case 'unclear': return <AlertTriangle className="h-4 w-4 text-yellow-600" />;
  ...
  const getRelevanceColor = (relevance: string) => {
    switch (relevance) {
      ...
      case 'contextual': return 'bg-gray-100 text-gray-800';
    ...
    <div className={`space-y-6 ${className}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center p-3 rounded-lg bg-green-50">
              <div className="text-2xl font-bold text-green-600">
                {provisions.flatMap(p => p.constitutionalBasis).filter(cb => cb.impact === 'supports').length}
              </div>
              <div className="text-sm text-muted-foreground">Supporting</div>
            </div>
            <div className="text-center p-3 rounded-lg bg-red-50">
              <div className="text-2xl font-bold text-red-600">
                {provisions.flatMap(p => p.constitutionalBasis).filter(cb => cb.impact === 'conflicts').length}
              </div>
              <div className="text-sm text-muted-foreground">Conflicts</div>
            </div>
            <div className="text-center p-3 rounded-lg bg-yellow-50">
              <div className="text-2xl font-bold text-yellow-600">
                {provisions.flatMap(p => p.constitutionalBasis).filter(cb => cb.impact === 'unclear').length}
              </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Click on an article to highlight related provisions
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {provision.constitutionalBasis.map((cb, index) => (
                          <div key={index} className={`p-4 rounded-lg border ${getImpactColor(cb.impact)}`}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Learn more about the constitutional principles involved
          </CardDescription>
        </CardHeader>
        <CardContent>
