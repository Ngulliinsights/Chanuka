...
export const generateActivityItems = (count: number = 50): ActivityItem[] => {
  const activityTypes: Array<'comment' | 'discussion' | 'expert_contribution' | 'bill_save' | 'bill_share' | 'campaign_join' | 'petition_sign'> = 
    ['comment', 'discussion', 'expert_contribution', 'bill_save', 'bill_share', 'campaign_join', 'petition_sign'];
  ...
  return Array.from({ length: count }, () => {
    ...
    const isExpertActivity = type === 'expert_contribution' || faker.datatype.boolean({ probability: 0.3 });
    
    const user = isExpertActivity && allExperts.length > 0 ? 
      faker.helpers.arrayElement(allExperts) : 
      faker.helpers.arrayElement(allUsers);

    const expertInfo = isExpertActivity && allExperts.some(e => e.id === user.id) ? {
    ...
    const content = type === 'comment' || type === 'discussion' || type === 'expert_contribution' ? 
      generateCommentContent(isExpertActivity) : undefined;
...
export const generateTrendingTopics = (count: number = 15): TrendingTopic[] => {
  const categories: Array<'bill' | 'policy_area' | 'campaign' | 'general'> = 
    ['bill', 'policy_area', 'campaign', 'general'];
...
export const generateExpertInsights = (count: number = 20): ExpertInsight[] => {
  ...
  return Array.from({ length: count }, () => {
    ...
    return {
      ...
      methodology: 'Analysis based on constitutional precedent, policy research, and stakeholder consultation.',
...
export const generateCampaigns = (count: number = 12): Campaign[] => {
  const campaignTypes: Array<'advocacy' | 'petition' | 'awareness' | 'action'> = 
    ['advocacy', 'petition', 'awareness', 'action'];
  ...
  return Array.from({ length: count }, () => {
    ...
    const currentCount = status === 'active' ? 
    ...
    return {
      ...
      endDate: status === 'active' ? 
        new Date(Date.now() + faker.number.int({ min: 30, max: 180 }) * 24 * 60 * 60 * 1000).toISOString() :
...
export const generatePetitions = (count: number = 8): Petition[] => {
  const statuses: Array<'active' | 'successful' | 'closed' | 'expired'> = 
    ['active', 'successful', 'closed', 'expired'];
  ...
  return Array.from({ length: count }, () => {
    ...
    const currentSignatures = status === 'active' ? 
      faker.number.int({ min: Math.floor(goal * 0.1), max: Math.floor(goal * 0.8) }) :
      status === 'successful' ? faker.number.int({ min: goal, max: goal * 1.5 }) :
    ...
    return {
      ...
      deadline: status === 'active' ? 
        new Date(Date.now() + faker.number.int({ min: 30, max: 90 }) * 24 * 60 * 60 * 1000).toISOString() :
        undefined,
      updatedAt: generateDateInRange(7, 0)
    };
  });
...
export const generateCommunityStats = (): CommunityStats => {
  ...
  return {
    totalMembers,
    activeToday: faker.number.int({ min: Math.floor(totalMembers * 0.01), max: Math.floor(totalMembers * 0.05) }),
    activeThisWeek: faker.number.int({ min: Math.floor(totalMembers * 0.1), max: Math.floor(totalMembers * 0.3) }),
 ...
 */
export const generateLocalImpactMetrics = (location?: { state?: string; district?: string; county?: string }): LocalImpactMetrics => {
