...
export const generatePersonalEngagementScore = (userId: string): PersonalEngagementScore => {
  ...
  return {
    ...
    methodology: {
      description: 'Civic engagement score calculated based on participation frequency, contribution quality, demonstrated expertise, and community impact.',
      factors: [
        {
          name: 'Participation',
          weight: 0.3,
...
export const generateCommunitysentimentAnalysis = (): CommunitysentimentAnalysis => {
  ...
  const topics = [
    'Healthcare Reform', 'Education Funding', 'Environmental Policy',
    'Criminal Justice', 'Infrastructure', 'Economic Policy'
  ];
  ...
  const polls = Array.from({ length: 3 }, () => {
    ...
    return {
      ...
      responses,
      results,
      endTime: new Date(Date.now() + faker.number.int({ min: 1, max: 7 }) * 24 * 60 * 60 * 1000).toISOString()
    };
  });

  return {
    overall: positive > negative + 0.1 ? 'positive' : negative > positive + 0.1 ? 'negative' : 'neutral',
    distribution: {
      positive,
      neutral,
      negative
...
export const generateEngagementStatistics = (): EngagementStatistics => {
  const leaderboard = faker.helpers.arrayElements(mockUsers, 10).map((user, index) => ({
    ...
    rank: index + 1,
    badge: faker.helpers.arrayElement(['Civic Champion', 'Policy Expert', 'Community Leader', 'Rising Star', 'Engaged Citizen']),
    contributions: {
  ...
  const achievements = [
    { id: 'first_comment', name: 'First Comment', description: 'Posted your first comment', icon: '💬', rarity: 'common' },
    { id: 'expert_recognition', name: 'Expert Recognition', description: 'Received expert verification', icon: '🎓', rarity: 'rare' },
    { id: 'community_leader', name: 'Community Leader', description: 'Led 5 successful campaigns', icon: '👑', rarity: 'epic' },
    { id: 'policy_influencer', name: 'Policy Influencer', description: 'Influenced major policy decision', icon: '⚖️', rarity: 'legendary' },
    { id: 'civic_champion', name: 'Civic Champion', description: 'Top 1% civic engagement score', icon: '🏆', rarity: 'legendary' }
  ].map(achievement => ({
    ...achievement,
...
export const generateUserEngagementProfile = (userId: string): UserEngagementProfile => {
  ...
  const specializations = faker.helpers.arrayElements([
    'Healthcare Policy', 'Environmental Law', 'Education Reform',
    'Criminal Justice', 'Economic Policy', 'Civil Rights'
...
export const generateCivicEngagementGoals = (count: number = 5): CivicEngagementGoal[] => {
  const goalTemplates = [
    { name: 'Weekly Participation', description: 'Participate in civic discussions 5 times this week', category: 'participation' as const },
    { name: 'Quality Contributions', description: 'Maintain 80% upvote ratio on comments', category: 'quality' as const },
    { name: 'Community Building', description: 'Help 10 new users get started', category: 'community' as const },
    { name: 'Expert Recognition', description: 'Earn expert verification in your field', category: 'expertise' as const },
    { name: 'Policy Impact', description: 'Contribute to 3 policy discussions', category: 'participation' as const }
  ];
...
export const generateEngagementNotifications = (count: number = 10): EngagementNotification[] => {
  ...
  const templates = {
    achievement: [
      ...
      { title: 'New Badge Earned!', message: 'Congratulations on becoming a "Quality Contributor"' }
    ],
    milestone: [
    ...
    community_update: [
      { title: 'Community Update', message: 'New bill discussion started in your area of interest' },
  ...
  return Array.from({ length: count }, () => {
    ...
    return {
      ...
      data: {
        billId: faker.datatype.boolean({ probability: 0.6 }) ? faker.number.int({ min: 1, max: 75 }) : undefined,
...
export const mockEngagementTrends = generateEngagementTrends(['This Week', 'Last Week', 'This Month', 'Last Month']);