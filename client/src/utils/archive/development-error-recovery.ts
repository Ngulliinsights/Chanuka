...
export class DevelopmentErrorRecovery {
  ...
  private async attemptHMRRecovery(_errorMessage: string): Promise<void> {
  ...
  private handleError(errorInfo: unknown): void {
    this.errorCount++;
    this.lastErrorTime = Date.now();

    const type = (errorInfo as { type?: string }).type || 'unknown';

    // Enhanced error logging
    console.group(`🚨 Development Error #${this.errorCount} [${type}]`);
    logger.error('Error Info', { component: 'DevelopmentErrorRecovery', error: errorInfo } as Record<string, unknown>);
    logger.error('Dev Server Info', { component: 'DevelopmentErrorRecovery', devServer: window.__DEV_SERVER__ } as Record<string, unknown>);
    logger.error('Recovery Attempts', { component: 'DevelopmentErrorRecovery', attempts: this.recoveryAttempts } as Record<string, unknown>);
    logger.error('Timestamp', { component: 'DevelopmentErrorRecovery', timestamp: new Date().toISOString() } as Record<string, unknown>);
    console.groupEnd();
  ...
  private shouldAttemptRecovery(errorInfo: unknown): boolean {
    ...
    const type = (errorInfo as { type?: string }).type;
    return typeof type === 'string' && recoverableTypes.includes(type);
  }

  private async attemptErrorRecovery(errorInfo: unknown): Promise<void> {
    if (this.isRecovering) return;
  ...
  private async recoverFromResourceError(errorInfo: unknown): Promise<void> {
    const info = errorInfo as Record<string, unknown>;
    const element = info.element as HTMLElement | undefined;
    const resourceUrl = info.resourceUrl as string | undefined;
    const resourceType = info.resourceType as string | undefined;
  ...
  private async recoverFromChunkError(_errorInfo: unknown): Promise<void> {
    logger.info('🔄 Attempting chunk error recovery', { component: 'DevelopmentErrorRecovery' } as Record<string, unknown>);
  ...
  private async recoverFromGeneralError(_errorInfo: unknown): Promise<void> {
    logger.info('🔄 Attempting general error recovery', { component: 'Chanuka' } as Record<string, unknown>);
  ...
  private storeErrorForDebugging(errorInfo: unknown): void {
    try {
      const errorLog = {
        error: errorInfo,
