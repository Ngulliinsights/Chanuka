...
export const ERROR_MESSAGE_TEMPLATES: ErrorMessageTemplate[] = [
  ...
  // Validation Errors
  {
    ...
    title: 'Invalid Input',
    message:
      'The information you provided contains errors. Please review and correct the highlighted fields.',
...

  {
    ...
    title: 'Memory Issue',
    message:
      'The application is using too much memory. Please close other tabs or restart your browser.',
...
export function getTemplatesByDomain(domain: ErrorDomain): ErrorMessageTemplate[] {
  return ERROR_MESSAGE_TEMPLATES.filter(template => template.domain === domain).sort(
    (a, b) => a.priority - b.priority
  );
}

export function getTemplatesBySeverity(severity: ErrorSeverity): ErrorMessageTemplate[] {
  return ERROR_MESSAGE_TEMPLATES.filter(template => template.severity === severity).sort(
    (a, b) => a.priority - b.priority
  );
}
...
export function getBestMatchTemplate(
  ...
  const exactMatch = ERROR_MESSAGE_TEMPLATES.find(
    template => errorCode && template.id.includes(errorCode.toLowerCase())
  );
  ...
  const domainMatches = ERROR_MESSAGE_TEMPLATES.filter(template => template.domain === domain);
...
export const DEFAULT_LOCALIZED_MESSAGES: Record<string, LocalizedErrorMessage> = {
  'en-US': {
    ...
    translations: {
      ...
      'external-service-unavailable':
        'An external service we depend on is temporarily unavailable. Please try again later.',
      'business-rule-violation':
        'This operation cannot be completed due to business rules or constraints.',
  ...
  'es-ES': {
    ...
    translations: {
      'network-connection-failed':
        'No pudimos conectarnos al servidor. Por favor, verifica tu conexión a internet e inténtalo de nuevo.',
      'network-timeout':
        'La solicitud tardó demasiado en completarse. Esto puede deberse a una conexión lenta o problemas del servidor.',
      'auth-session-expired':
        'Tu sesión ha expirado. Por favor, inicia sesión nuevamente para continuar.',
      'auth-invalid-credentials':
        'El correo electrónico o la contraseña que ingresaste son incorrectos. Por favor, inténtalo de nuevo.',
      'auth-permission-denied':
        'No tienes permiso para acceder a este recurso o realizar esta acción.',
      'validation-invalid-input':
        'La información que proporcionaste contiene errores. Por favor, revisa y corrige los campos resaltados.',
      'database-connection-failed':
        'Estamos teniendo problemas para conectarnos a nuestra base de datos. Por favor, inténtalo de nuevo en unos minutos.',
      'system-unexpected-error':
        'Ocurrió un error inesperado. Nuestro equipo ha sido notificado y está trabajando para solucionarlo.',
      'external-service-unavailable':
        'Un servicio externo en el que dependemos no está disponible temporalmente. Por favor, inténtalo más tarde.',
      'business-rule-violation':
        'Esta operación no se puede completar debido a reglas o restricciones de negocio.',
  ...
  'fr-FR': {
    ...
    translations: {
      'network-connection-failed':
        "Nous n'avons pas pu nous connecter au serveur. Veuillez vérifier votre connexion Internet et réessayer.",
      'network-timeout':
        "La requête a pris trop de temps à s'exécuter. Cela peut être dû à une connexion lente ou à des problèmes de serveur.",
      ...
      'auth-permission-denied':
        "Vous n'avez pas la permission d'accéder à cette ressource ou d'effectuer cette action.",
      'validation-invalid-input':
        'Les informations que vous avez fournies contiennent des erreurs. Veuillez vérifier et corriger les champs mis en évidence.',
      'database-connection-failed':
        'Nous avons des problèmes pour nous connecter à notre base de données. Veuillez réessayer dans quelques minutes.',
      'system-unexpected-error':
        'Une erreur inattendue est survenue. Notre équipe a été informée et travaille pour la résoudre.',
      'external-service-unavailable':
        'Un service externe dont nous dépendons est temporairement indisponible. Veuillez réessayer plus tard.',
      'business-rule-violation':
        'Cette opération ne peut pas être effectuée en raison de règles ou de contraintes commerciales.',
      'cache-invalid':
        'Il y a eu un problème avec les données en cache. Nous allons essayer de charger des données fraîches.',
      'generic-error':
...
export function getLocalizedMessage(templateId: string, locale: string = 'en-US'): string {
  ...
  return (
    DEFAULT_LOCALIZED_MESSAGES['en-US'].translations[templateId] ||
