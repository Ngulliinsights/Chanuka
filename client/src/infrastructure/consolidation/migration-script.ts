...
export function findFilesImportingFrom(project: Project, modulePath: string): SourceFile[] {
  ...
  for (const sourceFile of project.getSourceFiles()) {
    ...
    for (const importDecl of imports) {
      ...
      if (!resolvedModule && !moduleSpecifier.startsWith('.')) {
        errors.push(`${sourceFile.getFilePath()}: Cannot resolve import '${moduleSpecifier}'`);
      }
    }
  }
