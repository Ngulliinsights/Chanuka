...
This module consolidates all WebSocket and real-time functionality that was previously scattered across the codebase into a single, well-organized location at `client/src/infrastructure/realtime/`. The consolidation follows FSD principles and provides a clean, type-safe API for real-time features.
...
```typescript
import { RealTimeService, realTimeService } from '@client/infrastructure/realtime';
