...
export const ClipboardButton: React.FC<ClipboardButtonProps> = React.memo(
  ({ text, children, className = '', onSuccess, onError }) => {
...
export const FullscreenButton: React.FC<FullscreenButtonProps> = React.memo(
  ({ targetRef, children, className = '', onEnterFullscreen, onExitFullscreen, onError }) => {
...
export function useWebWorkerFallback<T, R>(
  ...
  const supportsWebWorkers = useMemo(() => typeof Worker !== 'undefined', []);
...
export const NotificationFallback: React.FC<NotificationProps> = React.memo(
  ({ title, body, icon, onClick, onClose }) => {
    const [showFallback, setShowFallback] = useState(false);

    const supportsNotifications = useMemo(() => 'Notification' in window, []);
    ...
    return (
      ...
      >
        <div className="flex items-start">
          {icon && <img src={icon} alt="" className="w-8 h-8 mr-3 flex-shrink-0" />}
