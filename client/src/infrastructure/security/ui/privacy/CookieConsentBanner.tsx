...
const DEFAULT_CATEGORIES: Omit<CookieCategory, 'enabled'>[] = [
  ...
  },
  {
    ...
    description:
      'These cookies track your online activity to help advertisers deliver more relevant advertising or to limit how many times you see an ad. They may be set by us or by third-party providers whose services we use.',
    required: false,
...
export function CookieConsentBanner({
  ...
  const applyCookieSettings = useCallback(
    (consent: Record<string, boolean>) => {
  ...
  const saveConsentToStorage = useCallback(
    (consent: Record<string, boolean>) => {
      try {
  ...
  const handleAcceptAll = useCallback(() => {
    ...
    const consent = updatedCategories.reduce(
      (acc, cat) => {
  ...
  const handleAcceptSelected = useCallback(() => {
    const consent = categories.reduce(
      (acc, cat) => {
        acc[cat.id] = cat.enabled;
        return acc;
      },
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              >
                ...
                marketing efforts. You can customize your preferences below or accept our
                ...
                {categories.map(category => (
                  ...
                  >
                    ...
                    <div
                      ...
                      aria-label={`Cookies in ${category.name} category`}
                    >
