...
class MockDataService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (this.config.preloadKeys.length > 0) {
        await preloadData(this.config.preloadKeys as any);
      }
  ...
  private startRealTimeSimulation(): void {
    ...
    mockRealTimeSimulator.addEventListener((event) => {
      this.handleRealTimeEvent(event);
    });

    // Start the simulator
    mockRealTimeSimulator.start();
  ...
  private handleRealTimeEvent(event: RealTimeEvent): void {
    ...
    this.realTimeListeners.forEach(listener => {
      try {
        ...
        logger.error('Error in real-time event listener', {
          component: 'MockDataService',
          error: error instanceof Error ? error.message : 'Unknown error'
        });
      }
   ...
   */
  async loadData<T>(key: keyof typeof dataLoaders): Promise<T> {
    if (!this.initialized) {
      throw new Error('Mock Data Service not initialized');
    }

    try {
      return await dataLoaders[key]() as T;
    } catch (error) {
   ...
   */
  async loadBatchData(keys: (keyof typeof dataLoaders)[]): Promise<Record<string, any>> {
    if (!this.initialized) {
      throw new Error('Mock Data Service not initialized');
    }
  ...
  triggerRealTimeEvent(eventType: string): void {
    if (this.config.enableRealTimeSimulation) {
      mockRealTimeSimulator.triggerEvent(eventType as any);
    }
  }
  ...
  getRecentEvents(count: number = 10): RealTimeEvent[] {
    return mockRealTimeSimulator.getRecentEvents(count);
  }
...
/**
 * Export data loaders for direct access
 */
export { dataLoaders };
