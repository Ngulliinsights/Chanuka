...
export function validateRoutes(routes: RouteDefinition[]): {
  ...
  routes.forEach(route => {
    if (route.parentRoute) {
      ...
      if (!parentExists) {
        errors.push(`Orphaned child route: ${route.path} references non-existent parent ${route.parentRoute}`);
      }
    }
  });
 ...
 */
export function findBrokenLinks(routes: RouteDefinition[], links: typeof navigationLinks): string[] {
