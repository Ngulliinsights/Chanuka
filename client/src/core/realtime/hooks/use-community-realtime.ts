...
export function useCommunityRealTime(): CommunityRealTimeHookReturn {
  ...
  const subscribeToDiscussion = useCallback((billId: number) => {
    ...
    try {
      ...
      logger.error('Failed to subscribe to discussion', {
        component: 'useCommunityRealTime',
        discussionId,
        billId
      }, error);
  ...
  const unsubscribeFromDiscussion = useCallback((billId: number) => {
    ...
    try {
      ...
      logger.error('Failed to unsubscribe from discussion', {
        component: 'useCommunityRealTime',
        discussionId,
        billId
      }, error);
  ...
  const sendCommentUpdate = useCallback((billId: number, commentData: Record<string, unknown>) => {
    try {
      ...
      logger.error('Failed to send comment update', {
        component: 'useCommunityRealTime',
        billId,
        commentData
      }, error);
  ...
  const sendVoteUpdate = useCallback((billId: number, voteData: Record<string, unknown>) => {
    try {
      ...
      logger.error('Failed to send vote update', {
        component: 'useCommunityRealTime',
        billId,
        voteData
      }, error);
  ...
  useEffect(() => {
    ...
    const syncInterval = setInterval(() => {
      try {
        ...
        logger.error('Error syncing with community service', {
          component: 'useCommunityRealTime'
        }, error);
      }
    ...
    return () => {
      ...
      for (const discussionId of subscribedDiscussions) {
        try {
          ...
          logger.error('Error unsubscribing from discussion on unmount', {
            component: 'useCommunityRealTime',
            discussionId
          }, error);
        }
