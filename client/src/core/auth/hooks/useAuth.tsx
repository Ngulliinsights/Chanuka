  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      try {
        ...
        // In development mode, this is expected behavior - just log as info
        if (process.env.NODE_ENV === 'development') {
          logger.info('Token validation skipped in development mode', {
            component: 'AuthProvider',
            reason: 'No backend connection expected',
          });
        } else {
          logger.warn('Token validation failed, continuing with guest session:', {
            component: 'AuthProvider',
            error: errorMessage,
          });
        }
