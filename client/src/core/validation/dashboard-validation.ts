...
import type { ActionItem, TrackedTopic, DashboardData, DashboardConfig } from '@client/shared/types/dashboard';
...
export function validateTrackedTopic(topic: TrackedTopic): void {
  ...
  const validCategories = ['healthcare', 'education', 'environment', 'economy', 'security', 'infrastructure', 'social', 'other'];
...
export function validateDashboardData(data: DashboardData): void {
  ...
  data.actionItems.forEach((item, index) => {
    try {
      ...
      throw new Error(`Invalid action item at index ${index}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  });
  ...
  data.trackedTopics.forEach((topic, index) => {
    try {
      ...
      throw new Error(`Invalid tracked topic at index ${index}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  });
}
...
export function safeValidateDashboardConfig(config: DashboardConfig): { success: boolean; data: DashboardConfig; error?: string } {
  try {
