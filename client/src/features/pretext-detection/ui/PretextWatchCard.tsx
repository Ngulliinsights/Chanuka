...
export const PretextWatchCard = React.memo<PretextWatchCardProps>(
  ({ score, onViewDetails, onTakeAction, onReportIssue }) => {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge variant={risk.color as 'destructive' | 'secondary' | 'default'}>
                {risk.level} Risk
              </Badge>
            ...
            {Object.entries(score.indicators).map(([key, indicator]) => (
              <div key={key} className="flex items-center gap-2 p-2 bg-muted rounded">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">Score: {indicator.score}</div>
                </div>
              </div>
            ))}
          </div>
