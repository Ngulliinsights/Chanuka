...
export function useJourneyTracker(session_id?: string, user_id?: string) {
  ...
  const startJourney = useCallback(
    (user_role?: UserRole | string) => {
      ...
      const role = user_role || UserRole.Public;
      const mappedRole = mapToJourneyRole(role as UserRole | 'public');
   ...
   * Map navigation UserRole to journey tracker UserRole
   */
  function mapToJourneyRole(role: UserRole | 'public' | string): 'admin' | 'expert' | 'citizen' | 'guest' {
    const roleStr = String(role).toLowerCase();
    if (roleStr === 'admin' || roleStr === UserRole.Admin) return 'admin';
    if (roleStr === 'expert' || roleStr === UserRole.Expert) return 'expert';
    if (roleStr === 'citizen' || roleStr === 'user' || roleStr === UserRole.Citizen || roleStr === UserRole.VerifiedCitizen) return 'citizen';
    return 'guest';
  }
