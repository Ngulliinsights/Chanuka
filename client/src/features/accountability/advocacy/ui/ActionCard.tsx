...
import type { ActionItem } from '../hooks/use-advocacy';
...
export function ActionCard({ action, onStart, onComplete, onView }: ActionCardProps) {
  ...
  return (
    <Card
      className={`hover:shadow-md transition-shadow ${isOverdue && action.status === 'pending' ? 'border-red-300' : ''}`}
    >
      <CardHeader>
        <div className="flex items-start justify-between">
          ...
          {action.status === 'completed' && <CheckCircle className="h-6 w-6 text-green-600" />}
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2 pt-2">
          {action.status === 'pending' && onStart && (
            <Button className="flex-1" onClick={() => onStart(action.id)}>
              <ArrowRight className="h-4 w-4 mr-2" />
              Start Action
            </Button>
          )}
          {action.status === 'in_progress' && onComplete && (
            <Button className="flex-1" onClick={() => onComplete(action.id)}>
              <CheckCircle className="h-4 w-4 mr-2" />
              Mark Complete
            </Button>
          )}
          {onView && (
            <Button variant="outline" className="flex-1" onClick={() => onView(action.id)}>
              View Details
            </Button>
          )}
        </div>
