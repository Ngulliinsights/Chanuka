...
export function useConstitutionalAnalysis(billId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await globalApiClient.get(`/api/constitutional-intelligence/bill/${billId}`);
...
export function useAnalyzeBill() {
  ...
  return useMutation({
    mutationFn: async (request: AnalyzeRequest) => {
      ...
      const response = await globalApiClient.post('/api/constitutional-intelligence/analyze', request);
      return response.data.analysis as ConstitutionalAnalysis;
    },
    onSuccess: (data) => {
...
export function useConstitutionalStatistics() {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await globalApiClient.get('/api/constitutional-intelligence/statistics');
