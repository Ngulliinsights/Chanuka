    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[CommunityStats] Component rendered with stats:', {
        ...
        activityRate: stats?.totalMembers
          ? (((stats?.activeToday ?? 0) / stats.totalMembers) * 100).toFixed(2) + '%'
          : 'N/A',
      ...
      <CardContent className="p-6">
        ...
        <div className="mt-6 p-4 bg-muted/30 rounded-lg border border-border/50">
          <div className="flex items-center justify-between text-sm mb-2">
            ...
            <span className={cn('font-semibold', activityLevel.color)}>{activityLevel.label}</span>
          </div>

          {/* Contextual message providing insights about current engagement levels */}
          <p className="text-xs text-muted-foreground leading-relaxed">{activitySummary}</p>
        </div>
      </CardContent>
    </Card>
  );
