    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <div className="text-2xl font-bold text-blue-900">{totalRequests.toLocaleString()}</div>
        </div>

        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="text-sm font-medium text-green-700 mb-1">Success Rate</div>
          <div className="text-2xl font-bold text-green-900">{(100 - errorRate).toFixed(2)}%</div>
        </div>

        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <div className="text-sm font-medium text-yellow-700 mb-1">Avg Response Time</div>
          <div className="text-2xl font-bold text-yellow-900">{avgResponseTime.toFixed(0)}ms</div>
        </div>

        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <div className="text-sm font-medium text-red-700 mb-1">Error Rate</div>
          <div className="text-2xl font-bold text-red-900">{errorRate.toFixed(2)}%</div>
          <div className="text-xs text-red-600 mt-1">{totalErrors.toLocaleString()} errors</div>
        </div>
      </div>
    </div>
  );
