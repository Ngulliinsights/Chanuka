...
export function ImplementationWorkaroundsTracker({ 
  ...
  const mockWorkarounds: ImplementationWorkaround[] = useMemo(() => [
    {
      id: 'wa-1',
      originalProvision: 'Mandatory disclosure of pharmaceutical industry investments above $10,000',
      ...
      description: 'Implemented through regulatory changes rather than legislative mandate, reducing transparency requirements'
    },
    {
      id: 'wa-2',
      ...
      description: 'Reclassified lobbying activities as consulting to circumvent cooling-off period'
    },
    {
      id: 'wa-3',
  ...
  const analysisData = useMemo(() => {
    ...
    const interestGroups = allWorkarounds.reduce((acc, workaround) => {
    ...
    Object.values(interestGroups).forEach((group: any) => {
      group.avgEffectiveness = group.workarounds.reduce((sum: number, w: ImplementationWorkaround) => 
        sum + w.effectiveness, 0) / group.workarounds.length;
    });
    ...
    const riskMetrics = {
      ...
      avgEffectiveness: allWorkarounds.reduce((sum, w) => sum + w.effectiveness, 0) / allWorkarounds.length,
      highRiskWorkarounds: effectivenessGroups.high.length,
      recentWorkarounds: allWorkarounds.filter(w => {
        const monthsAgo = (Date.now() - new Date(w.implementationDate).getTime()) / (1000 * 60 * 60 * 24 * 30);
        return monthsAgo <= 12;
      }).length
    };
    ...
    return {
      effectivenessGroups,
      interestGroups: Object.values(interestGroups).sort((a: any, b: any) => b.totalWorkarounds - a.totalWorkarounds),
      timelineData,
      riskMetrics
    };
  }, [allWorkarounds]);
  ...
  const getEffectivenessBadge = (effectiveness: number): 'default' | 'secondary' | 'destructive' => {
  ...
  const pieData = [
    { name: 'High Effectiveness', value: analysisData.effectivenessGroups.high.length, fill: effectivenessColors.high },
    { name: 'Medium Effectiveness', value: analysisData.effectivenessGroups.medium.length, fill: effectivenessColors.medium },
    { name: 'Low Effectiveness', value: analysisData.effectivenessGroups.low.length, fill: effectivenessColors.low }
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="text-2xl font-bold">
              {analysisData.riskMetrics.recentWorkarounds}
            </div>
            <div className="text-xs text-muted-foreground">
              Last 12 months
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Breakdown of workaround effectiveness levels
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-64">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Analysis of workaround impact on transparency
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="p-4 border rounded-lg">
                    ...
                    <Progress 
                      value={(analysisData.effectivenessGroups.high.length / analysisData.riskMetrics.totalWorkarounds) * 100} 
                      className="h-2"
                    />
                  ...
                  <div className="p-4 border rounded-lg">
                    ...
                    <Progress 
                      value={(analysisData.effectivenessGroups.medium.length / analysisData.riskMetrics.totalWorkarounds) * 100} 
                      className="h-2"
                    />
                    <div className="text-sm text-muted-foreground mt-1">
                  ...
                  <div className="p-4 border rounded-lg">
                    ...
                    <Progress 
                      value={(analysisData.effectivenessGroups.low.length / analysisData.riskMetrics.totalWorkarounds) * 100} 
                      className="h-2"
                    />
                    <div className="text-sm text-muted-foreground mt-1">
        ...
        <TabsContent value="timeline" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Chronological view of workaround implementations
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={analysisData.timelineData}>
                    ...
                    <XAxis 
                      dataKey="index"
                      label={{ value: 'Implementation Order', position: 'insideBottom', offset: -5 }}
                    />
                    <YAxis 
                      domain={[0, 1]}
                  ...
                  <BarChart data={analysisData.interestGroups}>
                    ...
                    <Tooltip 
                      formatter={(value: number, name: string) => {
                        if (name === 'avgEffectiveness') return [formatPercentage(value), 'Avg Effectiveness'];
                        return [value, name];
                      }}
                      contentStyle={{ 
                        backgroundColor: 'hsl(var(--popover))',
                    ...
                    />
                    <Bar dataKey="totalWorkarounds" fill="hsl(var(--civic-transparency))" name="Total Workarounds" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
                    ...
                    {group.avgEffectiveness > 0.6 && (
                      <div className="p-2 bg-red-50 border border-red-200 rounded text-sm">
                        ...
                        <span className="text-red-800">
                          High-effectiveness workarounds may significantly impact transparency in this area
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div key={workaround.id} className="p-4 border rounded-lg">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex-shrink-0 ml-4 text-right">
                        <Badge variant={getEffectivenessBadge(workaround.effectiveness)} className="mb-2">
                    ...
                    {workaround.effectiveness > 0.7 && (
                      <div className="mt-3 p-2 bg-red-50 border border-red-200 rounded text-sm">
                        <AlertTriangle className="h-4 w-4 inline mr-2 text-red-600" />
                        <span className="text-red-800">
                          High-effectiveness workaround - significant impact on original transparency goals
                        </span>
                      </div>
                    )}
                  </div>
