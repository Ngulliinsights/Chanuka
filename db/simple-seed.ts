...
async function simpleSeed() {
  console.log('🌱 Starting simple comprehensive seed process...');
  ...
  try {
    ...
    await db.execute(`
      ...
      VALUES 
        (${userIds[4]}, 'System administrator ensuring platform integrity and transparency.', ARRAY['platform management', 'data governance', 'civic technology'], 'Nairobi, Kenya', 'Chanuka Platform', true),
        (${userIds[3]}, 'Legal analyst and constitutional law expert with 15 years experience in legislative analysis.', ARRAY['constitutional law', 'legislative analysis', 'governance', 'policy research'], 'Nairobi, Kenya', 'University of Nairobi School of Law', true),
        (${userIds[2]}, 'Concerned citizen interested in government transparency and accountability.', ARRAY['civic engagement', 'community organizing'], 'Mombasa, Kenya', 'Coastal Civic Society', true),
        (${userIds[1]}, 'Human rights advocate working on governance and transparency issues.', ARRAY['human rights', 'governance', 'advocacy', 'transparency'], 'Kisumu, Kenya', 'Transparency International Kenya', true),
        (${userIds[0]}, 'Investigative journalist covering governance and legislative affairs.', ARRAY['investigative journalism', 'political reporting', 'data analysis'], 'Nairobi, Kenya', 'The Standard Media Group', true);
    `);
    ...
    await db.execute(`
      ...
      VALUES 
        (${billIds[4]}, ${userIds[3]}, 'The cryptocurrency framework in Section 11 requires more robust consumer protection measures. Current provisions may not adequately address the risks associated with digital asset volatility and market manipulation.', 24, 3, true),
        (${billIds[4]}, ${userIds[2]}, 'As a citizen concerned about digital rights, I appreciate the focus on privacy protection in Part IV. However, the data localization requirements might create barriers for small businesses trying to compete globally.', 18, 7, false),
        (${billIds[3]}, ${userIds[0]}, 'The climate-smart agriculture programs in Part II align well with Kenya''s climate commitments. Implementation will require significant coordination between national and county governments.', 15, 4, true),
        (${billIds[2]}, ${userIds[1]}, 'Universal healthcare is a human right. This amendment addresses critical gaps in our current system, particularly for rural communities.', 42, 8, false),
        (${billIds[0]}, ${userIds[2]}, 'As a young Kenyan, I''m hopeful about the entrepreneurship support programs. However, we need to ensure these opportunities reach youth in rural areas, not just urban centers.', 35, 2, false);
    `);
    ...
    await db.execute(`
      ...
      VALUES 
        (${billIds[4]}, ${userIds[0]}, 5, 1, 2, 8.5),
        (${billIds[4]}, ${userIds[1]}, 8, 1, 3, 12.0),
        (${billIds[3]}, ${userIds[0]}, 7, 1, 3, 11.5),
        (${billIds[2]}, ${userIds[1]}, 6, 1, 2, 9.0),
        (${billIds[1]}, ${userIds[2]}, 7, 1, 4, 12.0);
    `);
    ...
    await db.execute(`
      ...
      VALUES 
        (${userIds[0]}, 'bill_update', 'Bill Status Update', 'Digital Economy Enhancement Act 2024 has moved to committee review', ${billIds[4]}, false),
        (${userIds[1]}, 'comment_reply', 'New Reply to Your Comment', 'Someone replied to your comment on Agriculture Modernization Act', ${billIds[3]}, true),
        (${userIds[2]}, 'bill_update', 'New Bill Published', 'Climate Change Adaptation and Resilience Act 2024 has been published for public comment', ${billIds[1]}, false),
        (${userIds[3]}, 'verification_status', 'Account Verification Complete', 'Your expert verification has been approved', null, true),
        (${userIds[4]}, 'bill_update', 'Bill Passed', 'Universal Healthcare Access Amendment Bill 2024 has been passed', ${billIds[2]}, false);
    `);

    console.log('✅ Simple comprehensive seed data creation completed successfully!');
...
simpleSeed()
  .then(() => {
    console.log('🎉 Simple seed process completed successfully!');
    process.exit(0);
  })
  .catch((error) => {
    console.error('💥 Simple seed process failed:', error);
    process.exit(1);
  });