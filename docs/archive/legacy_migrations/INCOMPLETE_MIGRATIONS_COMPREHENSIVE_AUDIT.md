...
- 20+ orphaned/misplaced files in client/src/lib/types/
...
### Evidence
- File: `client/src/lib/types/dashboard.legacy.ts` - **Marked as LEGACY**
...
### Impact:
- Dashboard types scattered across client/src/lib/types/
...
```
✅ dashboard.legacy.ts                  (in client/src/lib/types/)
...
| **Client Type Integration** | Consolidate 14+ duplicate types in client/ | ❌ STALLED (no progress) | client/src/core/api/types/, client/src/lib/types/ | Diverged implementations, 2+ conflicting defs |
| **Dashboard Type Migration** | Move dashboard types to @shared/types/dashboard/ | ❌ ABANDONED (midway) | client/src/lib/types/dashboard* | Scattered types, blocked cleanup |
