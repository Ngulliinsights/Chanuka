  ...
  try {
    ...
    const poolConfig = process.env.DATABASE_URL 
      ...
      : {
          ...
          database: process.env.DB_NAME || 'legislative_platform',
          ssl: false
        };
    ...
    for (const ext of extensionsResult.rows) {
      try {
        ...
        console.log(`   Dropped extension: ${ext.extname}`);
      } catch (error) {
        console.log(`   Warning: Could not drop extension ${ext.extname}: ${(error as Error).message}`);
      }
    }
