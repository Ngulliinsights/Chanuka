...
export default async function seedSimple() {
  ...
  const userIds = createdUsers.rows.map((row: any) => row.id);
  ...
  const sponsorIds = createdSponsors.rows.map((row: any) => row.id);
  ...
  const billIds = createdBills.rows.map((row: any) => row.id);
