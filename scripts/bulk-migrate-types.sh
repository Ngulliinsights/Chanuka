...
find client/src -type f \( -name "*.ts" -o -name "*.tsx" \) -print0 | while IFS= read -r -d '' file; do
  if grep -q "@client/types" "$file"; then
    ...
    perl -i -pe "s|from\s+['\"]@client/types['\"]|from '@client/shared/types'|g" "$file"
    perl -i -pe "s|from\s+['\"]@client/types/dashboard['\"]|from '@client/shared/types/dashboard'|g" "$file"
    perl -i -pe "s|from\s+['\"]@client/types/loading['\"]|from '@client/shared/types/loading'|g" "$file"
    perl -i -pe "s|from\s+['\"]@client/types/navigation['\"]|from '@client/shared/types/navigation'|g" "$file"
    perl -i -pe "s|from\s+['\"]@client/types/mobile['\"]|from '@client/shared/types/mobile'|g" "$file"
    perl -i -pe "s|from\s+['\"]@client/types/user-dashboard['\"]|from '@client/shared/types/user-dashboard'|g" "$file"
