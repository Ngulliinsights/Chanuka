...
import { type Bill, type NewBill } from '@server/infrastructure/schema/foundation';
...
export interface IBillRepository {
  ...
  /**
   * Creates multiple bills in a single batch operation
   * @param bills Array of bill data to create
   * @returns Promise resolving to array of created bills or error
   */
  createBatch(bills: NewBill[]): Promise<Result<Bill[], Error>>;

  /**
   * Updates multiple bills in a single batch operation
   * @param updates Array of bill updates with IDs
   * @returns Promise resolving to array of updated bills or error
   */
  updateBatch(updates: Array<{ id: string; data: Partial<NewBill> }>): Promise<Result<Bill[], Error>>;

  /**
   * Deletes multiple bills in a single batch operation
   * @param ids Array of bill IDs to delete
   * @returns Promise resolving to success result
   */
