...
export const CACHE_CONFIG = {
  ...
  keyPrefixes: {
    ...
    affiliations: (id: number) => `affiliations_v3_${id}`,
    sponsorStats: () => 'sponsor_stats_v3',
    disclosureStats: () => 'disclosure_stats_v3',
  ...
  ttl: {
    ...
    disclosureData: 15 * 60,       // 15 min   — disclosures updated regularly
    analyticsReport: 60 * 60,      // 1 hour   — expensive computations
    relationshipMap: 60 * 60,      // 1 hour   — complex network analysis
    affiliationData: 2 * 60 * 60,  // 2 hours  — affiliations change rarely
    statistics: 15 * 60,           // 15 min   — aggregate counts stay fresh
    dashboard: 5 * 60,             // 5 min    — needs to be current
    alerts: 7 * 24 * 60 * 60,      // 7 days   — historical alert data
    healthCheck: 30,               // 30 sec   — must be very fresh
  },
} as const;
...
export const API_CONFIG = {
  ...
  rateLimits: {
    standard: { windowMs: 15 * 60 * 1000, max: 100 }, // 100 per 15 min
    expensive: { windowMs: 15 * 60 * 1000, max: 20 },  // 20 per 15 min for analytics
  },
