...
const mockComprehensiveResult = { bill_id: mockBillId, analysisId: mockAnalysisId, timestamp: new Date(),
    ...
    transparency_score: { overall: 80, grade: 'B' as const, breakdown: { sponsorDisclosure: 80, legislativeProcess: 80, financialConflicts: 80, publicAccessibility: 80 } },
    publicInterestScore: { score: 75, assessment: 'High' as const, factors: { economicScoreNormalized: 70, socialScoreNormalized: 80, transparency_score: 80 } },
...
describe('Analysis API Routes', () => {
  ...
  describe('GET /api/analysis/bills/:bill_id/comprehensive', () => {
  ...
  describe('POST /api/analysis/bills/:bill_id/comprehensive/run', () => {
    ...
    it('should return 201 and the new analysis result when triggered by admin', async () => {
      ...
      expect(response.body.status).toBe('success');
       ...
       expect(response.body.data.analysis.bill_id).toBe(mockBillId);
       expect(billComprehensiveAnalysisService.analyzeBill).toHaveBeenCalledWith(mockBillId);
    });
