...
try {
  ...
  console.warn('Could not load external logger, using console fallback', err);
}
...
export class SMTPService extends BaseEmailService {
  ...
  constructor() {
    ...
    this.config = {
      ...
      fromEmail: process.env.FROM_EMAIL || 'noreply@simpletool.gov',
      fromName: process.env.FROM_NAME || 'SimpleTool',
      settings: {
        ...
        maxQueueSize: parseInt(process.env.EMAIL_MAX_QUEUE || '1000', 10),
      },
    };
  }
...
<style>
  ...
  static billStatusUpdate(
    ...
    const frontendUrl = process.env.FRONTEND_URL || 'https://simpletool.gov';
