...
export class DatabaseService {
   ...
   */
  private buildPoolConfig(config?: Partial<DatabaseConfig>): pg.PoolConfig {
    ...
    return {
      ...
      database: config?.database || process.env.DB_NAME || 'chanuka',
      max: config?.max || 20,
      idleTimeoutMillis: config?.idleTimeoutMillis || 30000,
      connectionTimeoutMillis: config?.connectionTimeoutMillis || 5000,
  ...
  private closeCircuitBreaker(): void {
    if (this.circuitBreakerState !== 'closed') {
      ...
      console.log('🟢 Circuit breaker closed - database requests resumed');
    }
  }
  ...
  private scheduleReconnection(): void {
    if (this.connectionAttempts >= this.retryConfig.maxAttempts) {
      console.log('🔄 Maximum retry attempts reached, will continue with fallback mode');
      return;
    }
    ...
    setTimeout(async () => {
      try {
        ...
        console.error('❌ Reconnection attempt failed:', (error as Error).message);
        this.scheduleReconnection();
      }
    }, delay);
  }
  ...
  async withFallback<T>(
    ...
    try {
      ...
      const executionTime = Date.now() - startTime;
      console.error(`❌ Operation failed for ${context}:`, (error as Error).message);
  ...
  async withTransaction<T>(
    ...
    while (attempts < maxSerializationRetries) {
      ...
      try {
        ...
        console.error(`❌ Transaction failed for ${context}:`, (error as Error).message);
        throw error;
      } finally {
        client.release();
      }
