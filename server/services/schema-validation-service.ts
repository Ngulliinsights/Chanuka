  ...
  private async repairTable(validation: ValidationResult): Promise<boolean> {
    if (validation.tableName === 'compliance_checks' && validation.missingColumns.includes('next_check')) {
      try {
        ...
        `);
        console.log('✅ Added next_check column to compliance_checks table');
        return true;
      } catch (error) {
        console.error('❌ Failed to add next_check column:', error);
        throw error;
      }
    }
    ...
    if (validation.tableName === 'security_audit_logs' && validation.missingColumns.includes('timestamp')) {
      try {
        ...
        `);
        console.log('✅ Added timestamp column to security_audit_logs table');
        return true;
      } catch (error) {
        console.error('❌ Failed to add timestamp column:', error);
        throw error;
      }
    }
    ...
    if (validation.tableName === 'threat_intelligence' && validation.missingColumns.includes('ip_address')) {
      try {
        ...
        `);
        console.log('✅ Added ip_address column to threat_intelligence table');
        return true;
      } catch (error) {
