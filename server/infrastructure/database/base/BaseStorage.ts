...
export abstract class BaseStorage<T> {
  ...
  constructor(options: StorageConfig = {}) {
    ...
    console.log(`${this.config.prefix} storage initialized with config:`, this.config);
  }
  ...
  protected async getCached<R>(
    ...
    try {
      ...
      this.cache.delete(cacheKey);
      console.error(`Failed to fetch data for key ${cacheKey}:`, error);
      throw error;
    }
  }
  ...
  protected async invalidateCache(patterns: string | string[]): Promise<void> {
    ...
    if (invalidatedCount > 0) {
      console.log(`Invalidated ${invalidatedCount} cache entries for patterns:`, patternArray);
    }
  }
  ...
  protected getDatabase(forWrite: boolean = false) {
    // For now, use the same database connection for both read and write
    // In a production environment, you would implement read/write splitting here
    return db;
  }
  ...
  private setupCacheCleanup(): void {
    ...
    this.cleanupInterval = setInterval(() => {
      ...
      if (this.cache.size > 0) {
        ...
        console.log(`Cache stats: ${stats.size}/${stats.maxSize} entries, ${stats.hitRate}% hit rate`);
      }
    }, 5 * 60 * 1000); // 5 minutes
  }
  ...
  async shutdown(): Promise<void> {
    ...
    console.log(`${this.config.prefix} storage shutdown completed`);
  }
